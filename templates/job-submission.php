...
<div id="jb-job-submission-form-wrapper" class="jb">
	...
	if ( ! is_user_logged_in() && JB()->options()->get( 'account-required' ) && ! JB()->options()->get( 'account-creation' ) ) {
		...
		if ( $edit ) {
			...
			if ( JB()->options()->get( 'job-salary' ) ) {
				$salary_type = $data['job_salary_type'];
				$amount_type = $data['job_amount_type'];
				$amount      = $data['job_amount'];
				$min_amount  = $data['job_min_amount'];
				$max_amount  = $data['job_max_amount'];
				$period      = $data['job_period'];
			}
		}
			...
			$job_details_fields = array_merge(
				...
				array(
					...
					array(
						...
						'value'       => $amount_type,
						'conditional' => array( 'job_salary_type', '!=', 'not' ),
					),
					array(
						'type'        => 'text',
						'required'    => true,
					...
					array(
						...
						'id'          => 'job_min_amount',
						'value'       => $min_amount,
						'conditional' => array( 'job_amount_type', '=', 'range' ),
					),
					array(
						'type'        => 'text',
						'required'    => true,
