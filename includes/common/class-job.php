...
if ( ! class_exists( 'jb\common\Job' ) ) {
	...
	class Job {
		 ...
		 *
		 * @param int  $job_id Job post ID
		 * @param bool $raw    Return raw status or status label. Since 1.3.6
		 *
		 * @return string
		 *
		 * @since 1.0
		 * @since 1.3.6 Added $raw param.
		 */
		public function get_status( $job_id, $raw = false ) {
			$job = get_post( $job_id );
			...
			$status_label = ! empty( $post_status->label ) ? $post_status->label : '';
			if ( 'publish' === $job->post_status ) {
